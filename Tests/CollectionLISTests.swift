...
final class CollectionLISTests: XCTestCase {
  func test1() {
    ...
    XCTAssertEqual(
      array.longestIncreasingSubsequenceIndexes(including: []).map { array[$0] },
      [0, 2, 3, 4, 5, 7, 8, 9]
    )

    XCTAssertEqual(
      array.longestIncreasingSubsequenceIndexes(including: [3, 8]).map { array[$0] },
      [0, 1, 4, 5, 6, 9]
    )
  }
  ...
  func test2() {
    ...
    XCTAssertEqual(
      array.longestIncreasingSubsequenceIndexes(including: [0]).map { array[$0] },
      [2, 3, 4]
    )
  }
